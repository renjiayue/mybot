...
class BaseModel{
	...
	// 带过滤条件的精确单条查询
	// findOneByFilter(attributes, where){
	// 	return attributes ? this.model.findAll({attributes: attributes, where: where,row:true}) : this.model.findAll({where: where,row:true})
	// }
	// 带过滤条件的排序精确查询
	...
	update(attributes, where){
		// console.log('attributes',attributes,'where',where)
